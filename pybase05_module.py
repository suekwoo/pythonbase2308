# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 11:01:01 2023
...
# 모듈을 사용하기 위해서는 다음과 같은 문법을 이용하여 모듈을 임포트 해야 한다.
'''
1) import <모듈이름>
...
print([r.randint(0,1) for i in range(10)]) # 0, 1을 무작위로 10개 생성
print([r.randrange(11) for i in range(10)]) # 0~10을 무작위로 10개 생성
print([r.random() for i in range(3)])
print([round(r.random(),2) for i in range(3)]) # 0 ~ 1 사이 실수 생성
# (random.uniform(x,y), n) => x ~ y 사이의 수를 n자리까지!
print([round(r.uniform(-5,5),1) for i in range(10)]) 

lottoNum = []
for i in range(0, 6):
    lottoNum.append(r.randint(1, 46))
#     if i == 6:
#         lottoList[6] = f'+{r.randint(1, 46)}'
print(lottoNum)
